...
def add_continuity_constraints(prog, sigma, n, d, dt):
  ...
  for i in range(n-1):
      for k in range(1,5):
          cal = -factorial(k)*sigma[i+1][k]
          for j in range(k, d):
              cal+=(factorial(j)/factorial(j-k))*(dt[i]**(j-k))*sigma[i][j]
  
          prog.AddLinearEqualityConstraint(cal, np.zeros(2))
...
def add_minsnap_cost(prog, sigma, n, d, dt):
  ...
  for i in range(n):
      for j in range(4, d):
          ...
          for k in range(4, d):
              #cal+=(factorial(j)/factorial(j-k))*(dt[i]**(j-k))*sigma[i][j]
              cal+= ((factorial(j)*factorial(k))/(factorial(j-4)*factorial(k-4)))*np.transpose(sigma[i][j]) @ sigma[i][k]*(dt[i]**(j+k-7))/(j+k-7)
  prog.AddQuadraticCost(cal)
